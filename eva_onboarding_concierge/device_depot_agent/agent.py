...
try:
    ...
    from eva_onboarding_concierge.shared_libraries.constants import AgentNames, EquipmentTypes
except ImportError:
    ...
    try:
        ...
        from ..shared_libraries.constants import AgentNames, EquipmentTypes
    except ImportError:
...
def request_equipment(employee_name: str, employee_email: str, department: str,
    ...
    try:
        ...
        for item in equipment_list:
            ...
            # Find item in inventory with improved matching
            found_item = None
            
            # Normalize search terms
            item_type_normalized = item_type.replace(" ", "_").replace("-", "_")
            item_model_normalized = item_model.replace(" ", "_").replace("-", "_")
            ...
            for category, items in mock_inventory.items():
                ...
                category_match = (
                    item_type_normalized in category_normalized or 
                    category_normalized in item_type_normalized or
                    any(word in category_normalized for word in item_type_normalized.split("_")) or
                    any(word in item_type_normalized for word in category_normalized.split("_"))
                )
                
                if category_match:
                    # Look for model match within this category
                    for model_key, model_data in items.items():
                        model_key_normalized = model_key.replace(" ", "_").replace("-", "_")
                        model_name_normalized = model_data["model"].lower().replace(" ", "_").replace("-", "_")
                        
                        # Check multiple matching criteria
                        model_match = (
                            item_model_normalized in model_key_normalized or
                            model_key_normalized in item_model_normalized or
                            item_model_normalized in model_name_normalized or
                            model_name_normalized in item_model_normalized or
                            any(word in model_key_normalized for word in item_model_normalized.split("_") if len(word) > 2) or
                            any(word in model_name_normalized for word in item_model_normalized.split("_") if len(word) > 2)
                        )
            ...
            # If no exact match, try fuzzy matching on common terms
            if not found_item:
                common_mappings = {
                    "macbook": ["macbook_pro_14", "macbook_air_13"],
                    "laptop": ["macbook_pro_14", "macbook_air_13", "thinkpad_x1", "dell_latitude"],
                    "monitor": ["dell_27_4k", "lg_24_fhd"],
                    "mouse": ["wireless_mouse"],
                    "keyboard": ["mechanical_keyboard"],
                    "webcam": ["webcam"],
                    "headset": ["headset"],
                    "dock": ["docking_station"],
                    "docking": ["docking_station"],
                    "iphone": ["iphone_15_pro", "iphone_15"],
                    "phone": ["iphone_15_pro", "iphone_15"]
                }
                
                for search_term, model_keys in common_mappings.items():
                    if search_term in item_model_normalized or search_term in item_type_normalized:
                        # Find the first available item from the mapping
                        for category, items in mock_inventory.items():
